...
- 顶部 **权限审核** 可查看所有已导入的 Codex 权限审核，按允许、拒绝、未知筛选，
  并搜索命令或审核原因。Codex 对话顶部也有 **权限审核**，只显示当前对话及其子任务的审核。
- 同一个审核会话中的多次审核会分别展示；展开详情可以核对审批请求与响应原文，
  也可以打开原对话或审核会话。只显示已保存并导入的记录，不把缺失结果当作已批准。
- 首次升级此功能需运行安装脚本完成全量重解析；已有数据会保留。之后新增审核沿用后台同步，
  页面也提供刷新按钮。
